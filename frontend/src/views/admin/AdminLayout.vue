<!--
  AdminLayout.vue - 管理端整体布局组件

  作用：作为管理端所有页面的外层容器，提供固定的侧边导航栏和顶部栏结构。
       使用 Element Plus 的 el-container 布局体系实现：
         - el-aside（左侧导航区）：展示管理端菜单（首页/用户管理/房源管理/订单管理）
         - el-header（顶部栏）：显示当前登录管理员姓名和退出按钮
...
<template>
  <el-container style="height:100vh">
    ...
    <el-aside width="200px" style="background:#2c3e50">
      <!-- 顶部品牌标识区 -->
      <div style="height:60px;line-height:60px;text-align:center;color:#fff;font-size:18px;font-weight:bold;background:#1a252f">⚙️ 管理端</div>
      <!-- 侧边导航菜单 -->
      <el-menu :default-active="route.path" router background-color="#2c3e50" text-color="#bfcbd9" active-text-color="#42b983">
        ...
        <el-menu-item index="/admin/houses">🏠 房源管理</el-menu-item>
